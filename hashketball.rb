...
def num_points_scored(player)
  game_hash.each do |key, value|
   value[:players].each do |playa, stats|
     if playa == player
       return stats[:points]
      end
    end
...
def find_biggest_size
  shoe_size_array = []
  game_hash.each do |team,team_hash|
    team_hash.each do |category, info|
  ...
  biggest = shoe_size_array.sort.reverse[0]
  biggest
end
...
def big_shoe_rebounds
  game_hash.each do |key, value|
   value[:players].each do |playa, stats|
     if stats[:shoe] == find_biggest_size
