...
def team_names
  result = []
  result.push(game_hash[:home][:team_name])
  result.push(game_hash[:home][:team_name])
  result
end
...
def player_numbers(team)
  ...
  else game_hash[:away][:players].each{ |player, stats|
    result.push(stats[:number])
    }
  end
  return result
...
game_hash[:away][:players].each{|player, stats|
  ...
  if game_hash[:home][:players][biggest_player] != nil
  return game_hash[:home][:players][biggest_player][:rebounds]
  else
    return game_hash[:away][:players][biggest_player][:rebounds]
  end
end
