...
def player_numbers(name)
  array = []
  if game_hash[:home][:team_name].include?(name) 
    game_hash[:home][:players].each_value do |value|
        array <<  value[:number] 
    end  
  else game_hash[:away][:team_name].include?(name) 
    game_hash[:away][:players].each_value do |value|
        array <<  value[:number]
    end  
  end
  array
...
def big_shoe_rebounds
  ...
  game_hash[:home][:players].each do |name, stat|
    if stat[:shoe] > largest_shoe
    largest_shoed_player = name
    largest_shoe = stat[:shoe]
    end
  end  
