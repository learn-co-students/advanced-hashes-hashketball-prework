    ...
    team_data.each do |attribute, data|
      if attribute == :players
        data.each do |player, stat_hash|
          if player == name
            stat_hash.each do |category, stat|
              if category == :points
                return stat
              end
            end
...
def player_numbers(team_name)
  ...
  game_hash.each do |location, team_data|
    team_data.each do|attribute, data|
      if data == team_name
        empty_hash[location] = team_data
      end
    end
  end
  empty_hash.each do |location, team_data|
    team_data.each do|attribute, data|
      if attribute == :players
        data.each do |player, stat_hash|
          stat_hash.each do |category, stat|
            if category == :number
              numbers_array.push(stat)
            end
          end
        end
      end
...
def big_shoe_rebounds
  ...
  name_string = ""
  empty_hash.each do |name, size|
    if size == empty_hash.values.max
      name_string = name
    end
  end
  game_hash.each do |location, team_data|
